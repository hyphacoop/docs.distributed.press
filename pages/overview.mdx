...
untars the files to a temporary folder on the server.
...
The main things you need to know is that Distributed Press keeps a long-running [Kubo](https://www.npmjs.com/package/go-ipfs)
...
This CID and accompanying IPNS entry is used to populate the `links` section:
...
Holepunch similarly uses [`localdrive`](https://www.npmjs.com/package/localdrive) which overs a mutable file system API that is similar to Hyperdrive.
